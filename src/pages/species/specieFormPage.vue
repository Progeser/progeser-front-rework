<template>
  <div class="ma-10" v-if="species">
    <v-row class="justify-space-between align-center">
      ...
      <v-btn color="error" @click="deleteSpecies()" class="align-self-center">
        {{ t('common.delete') }}
      </v-btn>
    </v-row>
    <v-col class="ma-5">
      ...
      <v-list class="border">
        <v-list-item
          v-for="(element, index) in filteredStages"
          :key="index"
        >
          <v-row class="d-flex align-center">
            ...
            <v-col cols="2" class="d-flex align-center">
              <v-btn @click="element._destroy = true" color="error">
    ...
    <v-row class="d-flex justify-space-between align-center">
      ...
      <v-btn
        @click="sendSpecies()"
        color="primary"
        :disabled="species.name.trim().length <= 0 || species.plant_stages.filter(stage => stage.name.trim().length <= 0 || stage.duration <= 0).length >= 1"
      >
        {{ t('common.send') }}
      </v-btn>
    </v-row>
...
const filteredStages = computed(() =>
  stages.value.filter((stage) => !stage._destroy)
);
